...
export function MusicPlayerVisual() {
  ...
  return (
    <div className="w-full bg-[#FFFFFF] rounded-2xl p-6 md:p-8 flex flex-col justify-between border border-black/[0.08] shadow-lg shadow-black/[0.02] relative overflow-hidden group">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 z-10 border-b border-black/[0.06] pb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-cyan-50 border border-cyan-200 flex items-center justify-center text-[#008899]">
          ...
          <div>
            ...
            <p className="text-[11px] font-mono-tech text-[#008899]">
              AUDIO DSP ENGINE &middot; 64-BAND FFT ANALYZER
            </p>
          </div>
        </div>

        <div className="flex items-center gap-2 px-2.5 py-1 rounded-full bg-cyan-50 border border-cyan-200 text-[10px] font-mono-tech text-[#008899]">
          <Radio className="w-3.5 h-3.5 text-[#008899] animate-pulse" />
      ...
      <div className="my-6 z-10 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
              ...
              return (
                <div
                  ...
                  className={`transition-all duration-500 flex items-center gap-3 ${
                    isCurrent
                      ? 'text-[#111111] text-base md:text-lg font-bold scale-[1.01] translate-x-2'
                      : 'text-slate-400 text-xs md:text-sm'
                  }`}
                >
                  <span className={`font-mono-tech text-[10px] ${isCurrent ? 'text-[#008899] font-bold' : 'text-slate-400'}`}>
                    {lyric.time}
                  </span>
        ...
        <div className="md:col-span-5 flex flex-col items-center md:items-end justify-center">
          <div className="flex items-end gap-[3px] h-24 p-3 bg-[#FAFAF8] rounded-xl border border-black/[0.08] w-full justify-between shadow-inner">
            {Array.from({ length: 24 }).map((_, i) => {
              ...
              return (
                <div
                  key={i}
                  className="w-1.5 bg-gradient-to-t from-[#008899] via-cyan-500 to-slate-800 rounded-t-sm transition-all duration-150"
                  style={{ height }}
                />
              );
            })}
          </div>
          <span className="text-[9px] font-mono-tech text-slate-500 mt-2">
      ...
      {/* Bottom Controls Bar */}
      <div className="z-10 bg-[#FAFAF8] rounded-xl p-4 border border-black/[0.08] flex flex-col gap-3">
        {/* Scrubber Progress Bar */}
        <div className="w-full flex items-center gap-3">
          <span className="text-[10px] font-mono-tech text-slate-500">01:24</span>
          <div className="flex-1 h-1.5 bg-[#E5E5E0] rounded-full overflow-hidden relative cursor-pointer group">
            <div
              className="h-full bg-gradient-to-r from-[#008899] to-[#111111] relative"
              style={{ width: `${trackProgress}%` }}
            >
              <div className="absolute right-0 top-1/2 -translate-y-1/2 w-2.5 h-2.5 rounded-full bg-[#111111] shadow-md" />
            </div>
          </div>
          <span className="text-[10px] font-mono-tech text-slate-500">03:45</span>
        </div>

        {/* Playback Controls */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-slate-600 text-xs font-mono-tech">
            <Volume2 className="w-4 h-4 text-[#008899]" />
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="p-1.5 text-slate-500 hover:text-[#111111] transition-colors cursor-pointer"
            ...
            <button
              ...
              className="w-9 h-9 rounded-full bg-[#111111] text-white flex items-center justify-center font-bold hover:bg-[#008899] transition-all shadow-md cursor-pointer"
          ...
          <span className="text-[10px] font-mono-tech text-[#008899]">
            UI ENGINE v2.0
          </span>
        </div>
      </div>
