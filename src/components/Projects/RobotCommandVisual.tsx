    ...
    <div className="w-full bg-[#FFFFFF] rounded-2xl p-6 md:p-8 flex flex-col justify-between border border-black/[0.08] shadow-lg shadow-black/[0.02] relative overflow-hidden group">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 z-10 border-b border-black/[0.06] pb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-emerald-50 border border-emerald-300 flex items-center justify-center text-emerald-800">
          ...
          <div>
            ...
            <p className="text-[11px] font-mono-tech text-emerald-700">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-black/5 hover:bg-black/10 border border-black/10 text-[11px] font-mono-tech text-slate-700 hover:text-[#111111] transition-colors cursor-pointer"
          >
            {isSimulating ? <RotateCcw className="w-3.5 h-3.5 text-emerald-700" /> : <Play className="w-3.5 h-3.5 text-emerald-700" />}
            <span>{isSimulating ? 'PAUSE ROVER' : 'RUN SIM'}</span>
          </button>

          <button
            onClick={resetGrid}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-[11px] font-mono-tech text-emerald-800 transition-colors cursor-pointer"
          >
      ...
      <div className="my-6 z-10 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
        ...
        <div className="md:col-span-7 flex flex-col items-center">
          <div className="grid grid-cols-8 gap-1.5 p-3 rounded-xl bg-[#FAFAF8] border border-black/[0.08] shadow-inner">
                ...
                return (
                  <button
                    ...
                    className={`w-7 h-7 sm:w-8 sm:h-8 rounded flex items-center justify-center text-[10px] font-mono-tech transition-all cursor-pointer ${
                      isRoverHere
                        ? 'bg-[#111111] text-white font-extrabold shadow-md shadow-black/30 scale-105 z-10'
                        : isStart
                        ...
                        : 'bg-white hover:bg-slate-100 border border-black/[0.06]'
                    }`}
                    title={
                      isRoverHere
          ...
          <span className="text-[10px] font-mono-tech text-slate-500 mt-2">
        ...
        <div className="md:col-span-5 flex flex-col gap-3">
          <div className="p-4 rounded-xl bg-white border border-black/[0.08] font-mono-tech text-xs space-y-2 shadow-sm">
            <div className="flex justify-between text-slate-500">
              <span>ROVER COORD:</span>
              <span className="text-[#111111] font-bold">[{currentRobotPos.r}, {currentRobotPos.c}]</span>
            </div>
            <div className="flex justify-between text-slate-500">
              <span>PATH NODES:</span>
              <span className="text-emerald-700 font-bold">{path.length} NODES</span>
            </div>
            <div className="flex justify-between text-slate-500">
              <span>A* LATENCY:</span>
              <span className="text-[#111111]">&lt; 1.8ms</span>
            </div>
            <div className="flex justify-between text-slate-500">
              <span>ALGORITHM:</span>
              <span className="text-slate-700">MANHATTAN HEURISTIC</span>
            </div>
          </div>

          <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-200 text-[11px] font-mono-tech text-emerald-800 flex items-center gap-2">
            <Navigation className="w-4 h-4 shrink-0 text-emerald-700 animate-pulse" />
      ...
      <div className="z-10 text-xs font-mono-tech text-slate-500 flex items-center justify-between border-t border-black/[0.06] pt-3">
        ...
        <span className="text-emerald-700 font-semibold">STATUS: PATH LOCKED</span>
      </div>
    </div>
  );
}
